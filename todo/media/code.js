$(document).ready(function(){
	...
	$('#note-submit').live('click',function(e){
		...
		{
			alert("gettext('The note text is too short!')")
			return false;
		}
		
		if($('.pr-author').val().length<1)
		{
			alert("gettext('You haven't entered your name!')")
			return false;
		}
		
		$(this).replaceWith('<img src="/media/1/global/indicator.gif" style="margin:30px auto;display:block" />');
		...
		$.post('/todo/post/',data,function(msg){
...
function make_draggable(elements)
{
	...
	elements.draggable({
		...
		stop:function(e,ui){
			...
			/* Sending the z-index and positon of the note to update_position via AJAX GET: */
			$.get('/todo/update_position/',{
				  x		: ui.position.left,
				  y		: ui.position.top,
				  z		: zIndex,
